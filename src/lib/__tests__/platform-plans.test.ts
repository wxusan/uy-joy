...
test("navigation audience helper keeps every role sidebar explicit", () => {
  ...
  assert.equal(roleCanSeeNavigationAudience("developer", ["finance"]), false);
  assert.equal(roleCanSeeNavigationAudience("developer", ["developer"]), true);
